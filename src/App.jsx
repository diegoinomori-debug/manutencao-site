        ...
        {page === "purchase" && (
          <div className="tableWrap">
            ...
            <table>
              <thead>
                <tr>
                  <th>部品名</th><th>部品番号</th><th>購入先</th><th>発注状況</th><th>発注日</th><th>入荷予定日</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((row) => (
            ...
            <div className="calendarLayout">
              <div className="calendarMain">
                ...
                <div className="selectedEvents">
                  ...
                  {calendarEvents.filter((event) => event.date === selectedDate).map((event) => (
              ...
              <div className="calendarSide">
                ...
                {calendarEvents.map((row) => (
                  <div key={row.id} className="calendarEditCard">
                    ...
                    <input value={row.title || ""} placeholder="タイトル" onChange={(e) => updateField("calendar", row.id, "title", e.target.value)} />
                    <select value={row.importance || "通常"} onChange={(e) => updateField("calendar", row.id, "importance", e.target.value)}>
                      <option value="通常">通常</option>
                      <option value="重要">重要</option>
                    </select>
                    <textarea placeholder="内容" value={row.detail || ""} onChange={(e) => updateField("calendar", row.id, "detail", e.target.value)} />
                    <input placeholder="担当者" value={row.owner || ""} onChange={(e) => updateField("calendar", row.id, "owner", e.target.value)} />
                    ...
                    <button className="deleteButton" onClick={() => removeItem("calendar", row.id)}><Trash2 size={16} /> 削除</button>
                  </div>
                ))}
              </div>
            </div>
                ...
                <tbody>
                  {factoryLogs.map((row) => (
                    <tr key={row.id}>
                      ...
                      <td><input value={row.repairTime || ""} onChange={(e) => updateField("factoryLogs", row.id, "repairTime", e.target.value)} /></td>
        ...
        {page === "report" && (
          <>
            <div className="header">
              <div>
                <h2>保全作業報告書</h2>
                <p>複数の報告書を保存して、過去トラブルを検索・確認できます。</p>
              </div>
            ...
            {filteredReports.map((row) => (
              <div className="tableWrap" key={row.id} style={{ marginTop: "20px" }}>
                ...
                <div className="reportGrid">
                  ...
                  <label>①不具合発生日時<input type="datetime-local" value={row.troubleDateTime || ""} onChange={(e) => updateField("maintenanceReports", row.id, "troubleDateTime", e.target.value)} /></label>
                  <label>②保全作業開始日時<input type="datetime-local" value={row.workStartDateTime || ""} onChange={(e) => updateField("maintenanceReports", row.id, "workStartDateTime", e.target.value)} /></label>
                  <label>③保全作業完了日時<input type="datetime-local" value={row.workEndDateTime || ""} onChange={(e) => updateField("maintenanceReports", row.id, "workEndDateTime", e.target.value)} /></label>
                  <label>④生産開始日時<input type="datetime-local" value={row.productionStartDateTime || ""} onChange={(e) => updateField("maintenanceReports", row.id, "productionStartDateTime", e.target.value)} /></label>
                  <label>⑤停止除外時間<input value={row.stopExclusionTime || ""} onChange={(e) => updateField("maintenanceReports", row.id, "stopExclusionTime", e.target.value)} placeholder="例: 00:30" /></label>
                  <label>⑥機能低下(%)<input type="number" value={row.functionDownRate || ""} onChange={(e) => updateField("maintenanceReports", row.id, "functionDownRate", e.target.value)} /></label>
                  <label>グループ名<input value={row.groupName || ""} onChange={(e) => updateField("maintenanceReports", row.id, "groupName", e.target.value)} /></label>
                ...
                <h3>リンク先</h3>
                <textarea value={row.link || ""} onChange={(e) => updateField("maintenanceReports", row.id, "link", e.target.value)} />

                <h3>再発防止</h3>
                <div className="reportGrid">
                  <label>再発防止区分<input value={row.recurrenceCategory || ""} onChange={(e) => updateField("maintenanceReports", row.id, "recurrenceCategory", e.target.value)} /></label>
                </div>
                ...
                <h3>流出防止</h3>
                <textarea value={row.outflowPrevention || ""} onChange={(e) => updateField("maintenanceReports", row.id, "outflowPrevention", e.target.value)} />

                <h3>変化点ランク / FP点検</h3>
                <div className="reportGrid">
                  <label>変化点ランク<input value={row.changeRank || ""} onChange={(e) => updateField("maintenanceReports", row.id, "changeRank", e.target.value)} /></label>
                  <label>FP点検<input value={row.fpCheck || ""} onChange={(e) => updateField("maintenanceReports", row.id, "fpCheck", e.target.value)} /></label>
                </div>
                ...
                <div className="reportGrid">
                  ...
                  <label>保全交換部品費・外注費<input value={row.partsCost || ""} onChange={(e) => updateField("maintenanceReports", row.id, "partsCost", e.target.value)} /></label>
                  <label>保全費用合計<input value={row.totalCost || ""} onChange={(e) => updateField("maintenanceReports", row.id, "totalCost", e.target.value)} /></label>
                </div>

                <h3>在庫</h3>
                <div className="reportGrid">
                  <label>保全交換部品<input value={row.replacedPart || ""} onChange={(e) => updateField("maintenanceReports", row.id, "replacedPart", e.target.value)} /></label>
                  <label>在庫数<input value={row.stockQty || ""} onChange={(e) => updateField("maintenanceReports", row.id, "stockQty", e.target.value)} /></label>
                  <label>リスト更新チェック<input type="checkbox" checked={!!row.stockCheck} onChange={(e) => updateField("maintenanceReports", row.id, "stockCheck", e.target.checked)} /></label>
                </div>
