          ...
          <>
            ...
            <div className="tableWrap">
              <table>
                <thead>
                  <tr>
                    <th>日付</th><th>タイトル</th><th>内容</th><th>担当者</th><th>画像URL</th><th></th>
                  </tr>
                </thead>
                <tbody>
                  {calendarEvents.map((row) => (
                    <tr key={row.id}>
                      <td><input type="date" value={row.date || ""} onChange={(e) => updateField("calendar", row.id, "date", e.target.value)} /></td>
                      <td><input value={row.title || ""} onChange={(e) => updateField("calendar", row.id, "title", e.target.value)} /></td>
                      <td><textarea value={row.detail || ""} onChange={(e) => updateField("calendar", row.id, "detail", e.target.value)} /></td>
                      <td><input value={row.owner || ""} onChange={(e) => updateField("calendar", row.id, "owner", e.target.value)} /></td>
                      <td>
                        <input value={row.image || ""} onChange={(e) => updateField("calendar", row.id, "image", e.target.value)} placeholder="画像URL" />
                        {row.image && <img src={row.image} alt="" width="120" />}
                      </td>
                      <td><button className="deleteButton" onClick={() => removeItem("calendar", row.id)}><Trash2 size={16} /></button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <table>
                <thead>
                  <tr>
                    ...
                    <th>処置内容</th><th>復旧時間</th><th>担当者</th><th>画像URL</th><th>備考</th><th></th>
                  </tr>
                </thead>
                <tbody>
                  {factoryLogs.map((row) => (
                    <tr key={row.id}>
                      ...
                      <td>
                        <input value={row.image || ""} onChange={(e) => updateField("factoryLogs", row.id, "image", e.target.value)} placeholder="画像URL" />
                        {row.image && <img src={row.image} alt="" width="120" />}
                      </td>
